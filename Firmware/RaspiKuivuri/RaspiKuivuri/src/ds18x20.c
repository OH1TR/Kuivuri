...
Author:   Martin Thomas <eversmith@...>   
...
int DS18X20ReadSensors(struct ds1820Measurement_t *values,int valuesCount)
{
	uint8_t id[OW_ROMCODE_SIZE], sp[DS18X20_SP_SIZE], diff;
	uint8_t i;
	uint16_t meas;
	...
	for( diff = OW_SEARCH_FIRST; diff != OW_LAST_DEVICE; )
	{
		...
		if( id[0] == DS18B20_FAMILY_CODE || id[0] == DS18S20_FAMILY_CODE || id[0] == DS1822_FAMILY_CODE )
		{
			...
			{
				if(memcmp(values[i].id,id,OW_ROMCODE_SIZE)==0)
				{
					values[i].celcius=cel;
					values[i].cel_frac_bits=cel_frac_bits;
				}
			}
		} // if meas-sensor
	} // loop all sensors
